...
export default function BakiPage() {
  const { bakiList, jomaList, totalBaki, totalJoma, addBaki, addJoma, editBaki, editJoma, deleteBaki, deleteJoma, toggleBakiStatus, toggleJomaStatus, loading } = useLedger();
  ...
  const openEdit = (item: any) => {
    setEditingId(item.id); setName(item.person_name); setAmount(String(item.amount)); setDate(item.date); setNote(item.note); setShowAdd(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      const entry = { person_name: name.trim(), amount: amt, date, note: note.trim() };
      tab === 'baki' ? await editBaki(editingId, entry) : await editJoma(editingId, entry);
    } else {
      tab === 'baki'
  ...
  const rawList = tab === 'baki' ? bakiList : jomaList;
  const list = rawList.filter(item => {
    if (statusFilter === 'all') return true;
    if (tab === 'baki') return statusFilter === 'active' ? item.status === 'unpaid' : item.status === 'paid';
    return statusFilter === 'active' ? item.status === 'pending' : item.status === 'returned';
  });
    ...
    <div className="pb-28 animate-fade-in">
      ...
      <div className="mx-4 mb-4">
        <div className={`rounded-2xl p-5 text-white relative overflow-hidden ${tab === 'baki' ? 'gradient-card-blue' : 'gradient-warning'}`}>
          ...
          <p className="text-[10px] text-white/60 mt-1">
            {rawList.filter(i => tab === 'baki' ? i.status === 'unpaid' : i.status === 'pending').length} active / {rawList.length} total
          </p>
        </div>
      </div>
      ...
      <div className="mx-4 mb-3 flex gap-1.5">
        {(['all', 'active', 'done'] as const).map(f => (
          <button key={f} onClick={() => setStatusFilter(f)}
            className={`px-4 py-2 text-xs font-semibold rounded-xl transition-all duration-200 ${statusFilter === f ? 'gradient-primary text-white shadow-sm' : 'bg-card text-muted-foreground border border-border/50'}`}>
      ...
      <div className="mx-4 space-y-2">
        {loading ? <p className="text-center text-sm text-muted-foreground py-10">Loading...</p> :
          list.length === 0 ? <p className="text-center text-sm text-muted-foreground py-10">No entries found</p> : null}
        {list.map((item, i) => {
          const isBaki = tab === 'baki';
          const isDone = isBaki ? item.status === 'paid' : item.status === 'returned';
          return (
            <div key={item.id} className={`flex items-center gap-3 p-3.5 rounded-2xl glass-card animate-slide-up ${isDone ? 'opacity-50' : ''}`} style={{ animationDelay: `${i * 0.04}s` }}>
              <button onClick={() => isBaki ? toggleBakiStatus(item.id) : toggleJomaStatus(item.id)} className="shrink-0">
                {isDone ? <CheckCircle2 className="w-7 h-7 text-success" /> : <Circle className="w-7 h-7 text-muted-foreground/40" />}
              </button>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-semibold text-foreground truncate ${isDone ? 'line-through' : ''}`}>{item.person_name}</p>
              ...
              <button onClick={() => openEdit(item)} className="p-1.5 rounded-lg hover:bg-accent text-muted-foreground"><Pencil className="w-3.5 h-3.5" /></button>
              <button onClick={() => isBaki ? deleteBaki(item.id) : deleteJoma(item.id)} className="p-1.5 rounded-lg hover:bg-destructive/10 text-destructive/70"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          );
        })}
      </div>
      ...
      <Dialog open={showAdd} onOpenChange={(open) => { setShowAdd(open); if (!open) resetForm(); }}>
        <DialogContent className="max-w-[90vw] rounded-3xl">
          <DialogHeader><DialogTitle className="text-lg">{editingId ? 'Edit' : 'Add'} {tab === 'baki' ? 'Baki Pabo' : 'Joma Debo'}</DialogTitle></DialogHeader>
          <div className="grid gap-3 mt-2">
            ...
            <Button onClick={handleSave} className="h-11 rounded-xl gradient-primary text-white border-0 font-semibold">{editingId ? 'Update' : 'Save'}</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
